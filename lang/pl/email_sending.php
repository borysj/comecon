...
function createMail($newBlogPost, $fullTitle, $sEmail, $sBlogName)
{
    ...
    $mail->CharSet = "UTF-8";
    if ($newBlogPost) {
        $mail->Subject = "Nowy post na blogu $sBlogName: $fullTitle";
    } else {
        $mail->Subject = "Nowy komentarz na blogu $sBlogName do wpisu \"$fullTitle\")";
    }
    return $mail;
}
 ...
 *
 * @param string $postID The identifier of the blog post
 * @param string $postFullTitle The full title of the blog post
 * @param string $postURL The URL of the blog post
 * @param string $commentTimestamp The comment timestamp (YYYY-MM-DD HH:MM:SS),
 * pass null if it is a notification about a new blog post, not about a new
 * comment
...
function sendNotifications(
    ...
    // Create a link to the comment blog post and get the proper subscriber file
    if ($commentTimestamp !== null) {
        if ($userName === null || $userName === "" && $userComment === null || $userComment === "") {
            exit(EXITMSG_NOTIFICATIONERROR . " ::: " . __FILE__ . ":" . __LINE__);
        }
        $commentTimestamp = str_replace(array(" ", "-", ":"), "", $commentTimestamp);
        $postURL = $postURL . "#" . $commentTimestamp;
        $filename = $postID . "-SUBS.txt";
        $text1 = "skomentowanego";
        $text2 = "komentarze";
        $newBlogPost = false;
    } else {
    ...
    $body1 = "<html><body><p><a href=\"$link\">Link do $text1 wpisu</a></p>";
    ...
    if ($commentTimestamp !== null) {
        ...
        $body1 = $body1 . "<p><b>Autor komentarza:</b> $nick</p>";
        $body1 = $body1 . "<p>$userComment</p><br><br>";
        $body1 = $body1 . "<p style=\"font-size: small;\">Powyżej widzisz pierwotną wersję komentarza.
                           Autor mógł go w międzyczasie zmienić albo usunąć.
                           Kliknij link, żeby przeczytać najnowszą wersję.</p>";
        ...
        if ($sEmail['ownerPrivateMail']) {
            ...
            $mail = createMail($newBlogPost, $postFullTitle, $sEmail, $sGeneral['blogName']);
    ...
    while (!feof($subscribers)) {
        ...
        $body2 = "<p style=\"font-size: small;\"><a href=\"$unsubLink\">
                  Użyj tego linku, że odsubskrybować $text2</a></p>
                  <p style=\"font-size: small;\">Nie odpowiadaj na ten mail.
                  W przypadku problemów technicznych napisz do mnie tutaj:
